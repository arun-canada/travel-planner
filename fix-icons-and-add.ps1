# Fix emoji icons and add quickAddToTrip function
$content = Get-Content 'app.js' -Encoding UTF8 -Raw

# Replace the icon definitions with proper text icons
$content = $content -replace "icon: '🌍'", "icon: '🌍'"
$content = $content -replace "icon: '💰'", "icon: '💰'"
$content = $content -replace "icon: '✈️'", "icon: '✈️'"
$content = $content -replace "icon: '🏨'", "icon: '🏨'"
$content = $content -replace "icon: '🎭'", "icon: '🎭'"
$content = $content -replace "icon: '📅'", "icon: '📅'"
$content = $content -replace "icon: '💪'", "icon: '💪'"

# Add quickAddToTrip function before the last line
$quickAddFunction = @"

// Quick add to trip function for assistant
function quickAddToTrip(destName) {
  if (state.trips.length === 0) {
    // Create a new trip with this destination
    const newTrip = {
      id: generateId(),
      name: ``Trip to ${destName.split(',')[0]}``,
    ...
    showToast(``Created trip to ${destName.split(',')[0]}!``, 'success');
    ...
    addDestinationFromChat(destName.split(',')[0], destName.split(',')[1] || '');
  }
}
...
# Find a good place to insert (before the last closing brace or at the end)
if ($content -match '(?s)(.*)(// Handle hash changes.*$)') {
  $content = $matches[1] +  $quickAddFunction + "`r`n`r`n" + $matches[2]
}

Set-Content -Path 'app.js' -Value $content -Encoding UTF8
Write-Output "Fixed icons and added trip functions"
